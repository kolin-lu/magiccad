...
- 「生成选项」中可开关**是否显示思考过程**，并可设置**生成精度**（快速/均衡/精细）：控制思考深度与建模细节程度——Claude 模型调整思考与精力参数，OpenAI 协议模型通过提示词约束
- 管理用户（授权 / 删除）、删除任意市场作品
...
- ✏️ **圈注反馈**：文字说不清的位置，点「圈注反馈」截取当前画布，用方框/箭头/画笔在图上标出要调整的部位（可配文字说明）发给 AI；上传的图片框选后也可转入圈注
- ⏳ **后台异步生成**：生成在服务端独立运行，关掉页面也不中断；重新打开自动续接进行中的任务，已完成的结果自动存入会话历史；生成中可一键**中止**，已生成的部分保留
